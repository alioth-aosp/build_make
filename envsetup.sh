...
EOF
    ...
    __print_bootleggers_functions_help
...
function lunch()
{
    ...
    then
        # if we can't find a product, try to grab it off the BootleggersROM-Devices GitHub
        T=$(gettop)
        cd $T > /dev/null
        vendor/bootleggers/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/bootleggers/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
